"use client";
import { briefcaseIconley } from "@/utils/assets";
      ...
      <div className="w-full">
        ...
        >
          {myExperience?.map((exp, index) => (
            ...
            >
              <div className="w-full grid grid-cols-[max-content_auto_max-content] flex-none items-center h-min flex-nowrap overflow-visible p-0 gap-4 relative">
                <div className="flex items-center flex-1 gap-3 h-min justify-start overflow-visible p-0 relative ">
                  <div className="bg-dark-gray-4 border border-border-color rounded-md flex float-none gap-[10px] h-min justify-center items-center overflow-visible relative w-min ">
                    <div className="flex-none aspect-square h-auto overflow-visible relative w-[40px] rounded-[inherit]">
                      <figure className="block absolute inset-0 rounded-[inherit]">
                        <Image
                          width={30}
                          height={30}
                          src={exp.logo}
                ...
                <div className="flex gap-2 flex-wrap h-min overflow-visible p-0 w-fit items-center justify-start">
                  {exp.label &&
                    exp.label.length &&
                    exp.label.map((labelItem, labelIndex) => (
                      <div
                        key={labelIndex}
                        className="opacity-0 md:opacity-100 bg-dark-gray-2 border border-dark-gray-3 flex items-center justify-start flex-nowrap gap-[10px] h-min overflow-visible p-[4px_10px] relative w-min rounded-3xl "
                      >
                        <div className="flex-none h-auto relative whitespace-pre w-auto flex flex-col justify-start flex-shrink-0">
                ...
                <div className="flex gap-[10px] h-min overflow-visible p-0 w-fit items-center">
